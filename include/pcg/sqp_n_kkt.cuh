...
auto sqpSolvePcg(const uint32_t solve_count, 
    ...
    for(uint32_t sqpiter = 0; sqpiter < SQP_MAX_ITER; sqpiter++){
        ...
        generate_kkt_submatrices_n<T><<<knot_points*solve_count, KKT_THREADS, 2 * get_kkt_smem_size<T>(state_size, control_size)>>>(
            solve_count,
            state_size,
            control_size,
            knot_points,
        ...
        for (uint32_t prob = 0; prob < solve_count; prob++) {
            ...
            //     std::cout << h_merit_news[i] << (i == 7 ? "\n" : " ");
                ...
                //printf("prob %d, index %d, merit %f\n", prob, i, h_merit_news[i]);
            }
