...
namespace grid {
    ...
    const int FD_DU_MAX_SHARED_MEM_COUNT_new_version = 2625 + 126+98+49+7;
    ...
    template <typename T>
    __device__
    void forward_dynamics_device(T *s_qdd, const T *s_q, const T *s_qd, const T *s_u, T *s_XITemp, const robotModel<T> *d_robotModel, const T gravity) {
    ...
    __device__
    void forward_dynamics_gradient_device(T *s_df_du, const T *s_q, const T *s_qd, const T *s_u, T *s_XITemp, const robotModel<T> *d_robotModel, const T gravity) {
        T *s_XImats = s_XITemp; T *s_vaf = &s_XITemp[504]; T *s_dc_du = &s_vaf[126]; T *s_Minv = &s_dc_du[49]; T *s_qdd = &s_Minv[49]; T *s_temp = &s_qdd[7];
        ...
        inverse_dynamics_inner<T>(s_temp, s_vaf, s_q, s_qd, s_XImats, &s_temp[7], gravity); __syncthreads();
