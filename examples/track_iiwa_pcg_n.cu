...
int main(){
    ...
    for(uint32_t ind = 0; ind < start_goal_combinations; ind++){
		...
		auto eePos_traj2d = readCSVToVecVec<linsys_t>(eePos_traj_file_name);
		
		snprintf(xu_traj_file_name, sizeof(xu_traj_file_name), "examples/trajfiles/%d_%d_traj.csv", start_state, goal_state);
		auto xu_traj2d = readCSVToVecVec<linsys_t>(xu_traj_file_name);
		...
		auto trackingstats = simulateMPC_n<linsys_t, toplevel_return_type>(
			state_size,
			control_size,
			knot_points, 
